...
var bits = _interopRequireWildcard(require("@webassemblyjs/leb128/lib/bits"));

var bufs = _interopRequireWildcard(require("@webassemblyjs/leb128/lib/bufs"));
...
function decodeInt64(encodedBuffer, index) {
  ...
  var good_buffer = bufs.alloc(8);

  if (result.value[result.value.length-1] >= 0x80) {
    good_buffer.fill(0xFF);
  } else {
    good_buffer.fill(0);
  }

  result.value.copy(good_buffer);

  var value = _long.default.fromBytesLE(good_buffer, false);

  bufs.free(result.value);
  bufs.free(good_buffer);
