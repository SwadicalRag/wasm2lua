...
export class WebIDLBinder {
    ...
    mangleArrayIndexerName(opName: "get" | "set" | "new" | "delete" | "len",arrTypeName: string) {
                    ...
                    if(member.type == "operation") {
                        for(let k=member.arguments.length-1;k >= 0;k--) {
                            if(member.arguments[k].optional) {
                                let copy: webidl.OperationMemberType = JSON.parse(JSON.stringify(member));
    ...
    convertLuaToCPP_Pre(buf: string[],arg: {name: string,idlType: webidl.IDLTypeDescription,extAttrs: webidl.ExtendedAttributes[]} | webidl.Argument,argID: number) {
        ...
        if(arg.idlType.idlType == "DOMString") {
            ...
            if(this.hasExtendedAttribute("ToWASMOwned",arg.extAttrs)) {
                ...
                // this.luaC.write(buf,`if ${arg.name}.__luaOwned then ${arg.name}.__luaOwned = false else error("Parameter ${arg.name} is not owned by Lua, and therefore does not have the ability to transfer its ownership to WASM.") end `);
            }
            else if(this.hasExtendedAttribute("ToLuaOwned",arg.extAttrs)) {
                // If this WASM function rejects ownership of the passed in Lua arg,
                // let the Lua arg know
    ...
    convertLuaToCPP_Post(buf: string[],arg: {name: string,idlType: webidl.IDLTypeDescription,extAttrs: webidl.ExtendedAttributes[]} | webidl.Argument,argID: number) {
        if(!this.getExtendedAttribute("ConvertInputArray",arg.extAttrs)) {
            ...
            // Only modifiable by ToLuaOwned
            // ToLuaOwned doesn't make sense here since these values are Lua owned by default
            let luaOwned = true;

            if(this.hasExtendedAttribute("ToWASMOwned",arg.extAttrs)) {
                // This WASM Function's argument's ownership is transferred to WASM
                luaOwned = false;
            }
            else if(this.hasExtendedAttribute("ToLuaOwned",arg.extAttrs)) {
    ...
    convertCPPToLuaReturn(buf: string[],argType: webidl.IDLTypeDescription,extAttrs: webidl.ExtendedAttributes[],argName: string) {
        // Returns from WASM are owned by WASM by default (so ToWASMOwned doesn't make sense here)
        let luaOwned = false;

        if(this.hasExtendedAttribute("ToLuaOwned",extAttrs)) {
            // Unless Lua accepts ownership
            luaOwned = true;
        }
    ...
    convertLuaToCPPReturn(buf: string[],argType: webidl.IDLTypeDescription,extAttrs: webidl.ExtendedAttributes[],argName: string) {
        ...
        else if(this.classLookup[argType.idlType as string]) {
    ...
    convertCPPToLua_Pre(buf: string[],arg: {name: string,idlType: webidl.IDLTypeDescription,extAttrs: webidl.ExtendedAttributes[]},argID: number) {
        ...
        if(this.classLookup[arg.idlType.idlType as string]) {
            // By default, arguments from WASM to Lua are managed by WASM.
            let luaOwned = false;
    ...
    startWrappedCReturnValue(buf: string[],idlType: webidl.IDLTypeDescription,extAttrs: webidl.ExtendedAttributes[]) {
        if(this.hasExtendedAttribute("Array",extAttrs)) {
            let arrType = this.getExtendedAttribute("Array",extAttrs) as webidl.ExtendedAttributes;
            if(arrType.rhs) {
            ...
            let arrType = this.getExtendedAttribute("PointerArray",extAttrs) as webidl.ExtendedAttributes;
            if(arrType.rhs) {
                this.cppC.write(buf,`new _LuaArray<${idlType.idlType}*>(${this.unquote(arrType.rhs.value)},`);
            }
            else {
    ...
    endWrappedCReturnValue(buf: string[],idlType: webidl.IDLTypeDescription,extAttrs: webidl.ExtendedAttributes[]) {
        if(this.hasExtendedAttribute("Array",extAttrs) || this.hasExtendedAttribute("PointerArray",extAttrs)) {
            this.cppC.write(buf,`)`);
        }
    }

    startWrappedCValue(buf: string[],idlType: webidl.IDLTypeDescription,extAttrs: webidl.ExtendedAttributes[]) {
        // only a suffix is needed
    }

    endWrappedCValue(buf: string[],idlType: webidl.IDLTypeDescription,extAttrs: webidl.ExtendedAttributes[]) {
        if(this.hasExtendedAttribute("Array",extAttrs) || this.hasExtendedAttribute("PointerArray",extAttrs)) {
    ...
    walkInterfaceLua(node: webidl.InterfaceType) {
        ...
        if(this.hasExtendedAttribute("NoDelete",node.extAttrs)) {
            // Except when they can't be deleted.
            // In that case they should not be GC'd at all
            classLuaOwned = false;
        }
